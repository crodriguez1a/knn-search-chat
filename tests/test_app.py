...
class TestApp:
    def setup_class(self):
        ...
        self.search_index: nmslib.dist.FloatIndex = create_index(keyphrase_embeddings)

    def test_can_known_questions(self):
        basketball: str = chat("What's the best team in the NBA?", self.qa_bot, self.search_index)
        hometown: str = chat("Where did you grow up?", self.qa_bot, self.search_index)
        baseball: str = chat("What's your favorite baseball team?", self.qa_bot, self.search_index)
    ...
    def test_unknown_answers(self):
        weather: str = chat("what's the weather", self.qa_bot, self.search_index)
